...
ENV \
  ...
  LC_ALL=C.UTF-8 \
  LC_CTYPE=C.UTF-8 \
  PATH=/root/.asterius-local-install-root/bin:/root/.asterius-snapshot-install-root/bin:/root/.asterius-compiler-bin:/root/.local/bin:/root/.nvm/versions/node/v14.9.0/bin:${PATH}
...
RUN \
  ...
  bash -c ". ~/.nvm/nvm.sh && nvm install 14.9.0" && \
