      ...
      Right postStartupMode ->
        ...
        GHC.runGhc mbMinusB $ do
          ...
          case postStartupMode of
            Left preLoadMode -> liftIO $ do
              case preLoadMode of
                ShowInfo -> showInfo dflags
                ShowGhcUsage -> showGhcUsage dflags
                ShowGhciUsage -> showGhciUsage dflags
                PrintWithDynFlags f -> putStrLn (f dflags)
            Right postLoadMode -> main' postLoadMode dflags argv3 flagWarnings

main' :: PostLoadMode -> DynFlags -> [Located String] -> [Warn] -> Ghc ()
main' postLoadMode dflags0 args flagWarnings = do
  ...
  let dflt_target = hscTarget dflags0
      (mode, lang, link) = case postLoadMode of
  ...
  let dflags1 =
      ...
      dflags2
        | DoInteractive <- postLoadMode = def_ghci_flags
  ...
  GHC.prettyPrintGhcErrors dflags4 $ do
    ...
    let -- To simplify the handling of filepaths, we normalise all filepaths right
        ...
        dflags5 =
          dflags4 {ldInputs = map (FileOption "") objs ++ ldInputs dflags4}
    ...
    case verbosity dflags6 of
      v
        | v == 4 -> liftIO $ dumpPackagesSimple dflags6
    ...
    handleSourceError
      ...
      $ do
        case postLoadMode of
          ...
          DoEval exprs -> ghciUI hsc_env dflags6 srcs $ Just $ reverse exprs
...
ghciUI ::
  HscEnv -> DynFlags -> [(FilePath, Maybe Phase)] -> Maybe [String] -> Ghc ()
...
looks_like_an_input m =
  isSourceFilename m || looksLikeModuleName m || "-" `isPrefixOf` m
    || not
      (hasExtension m)
...
checkOptions ::
  PostLoadMode -> DynFlags -> [(String, Maybe Phase)] -> [String] -> IO ()
...
checkOptions mode dflags srcs objs = do
  ...
  when (notNull (filter wayRTSOnly (ways dflags)) && isInterpretiveMode mode) $
    hPutStrLn
      stderr
  ...
  when
    ( (filter (not . wayRTSOnly) (ways dflags) /= interpWays)
...
verifyOutputFiles dflags = do
  ...
  where
    nonExistentDir flg dir =
      throwGhcException
        ( CmdLineError
            ( "error: directory portion of "
                ++ show dir
                ++ " does not exist (used with "
                ++ show flg
                ++ " option.)"
            )
        )
...
printSetting k = mkPreLoadMode (PrintWithDynFlags f)
  where
    f dflags =
      fromMaybe (panic ("Setting not found: " ++ show k)) $
...
parseModeFlags args = do
  let ((leftover, errs1, warns), (mModeFlag, errs2, flags')) =
        runCmdLine (processArgs mode_flags args) (Nothing, [], [])
      mode = case mModeFlag of
        Nothing -> doMakeMode
        Just (m, _) -> m

  -- See Note [Handling errors when parsing commandline flags]
  unless (null errs1 && null errs2) $
    throwGhcException $
      errorsToGhcException $
        map (("on the commandline",)) $
          map (unLoc . errMsg) errs1
            ++ errs2
...
mode_flags =
  [ ------- help / version ----------------------------------------------
    ...
    [ defFlag
        "-show-iface"
        (HasArg (\f -> setMode (showInterfaceMode f) "--show-iface")),
      ------- primary modes ------------------------------------------------
      defFlag
        "c"
        ( PassFlag
            ( \f -> do
                setMode (stopBeforeMode StopLn) f
                addFlag "-no-link" f
            )
        ),
      ...
      defFlag
        "-frontend"
        (SepArg (\s -> setMode (doFrontendMode s) "-frontend"))
    ]

setMode :: Mode -> String -> EwM ModeM ()
setMode newMode newFlag = liftEwM $ do
  (mModeFlag, errs, flags') <- getCmdLineState
  let (modeFlag', errs') = case mModeFlag of
        Nothing -> ((newMode, newFlag), errs)
        Just (oldMode, oldFlag) -> case (oldMode, newMode) of
          -- -c/--make are allowed together, and mean --make -no-link
          _
            | isStopLnMode oldMode
                && isDoMakeMode newMode
                || isStopLnMode newMode
                && isDoMakeMode oldMode ->
              ((doMakeMode, "--make"), [])
          -- If we have both --help and --interactive then we
          -- want showGhciUsage
          _
            | isShowGhcUsageMode oldMode && isDoInteractiveMode newMode ->
              ((showGhciUsageMode, oldFlag), [])
            | isShowGhcUsageMode newMode && isDoInteractiveMode oldMode ->
              ((showGhciUsageMode, newFlag), [])
          -- If we have both -e and --interactive then -e always wins
          _
            | isDoEvalMode oldMode && isDoInteractiveMode newMode ->
              ((oldMode, oldFlag), [])
            | isDoEvalMode newMode && isDoInteractiveMode oldMode ->
              ((newMode, newFlag), [])
            | -- Otherwise, --help/--version/--numeric-version always win
              isDominantFlag oldMode ->
              ((oldMode, oldFlag), [])
            | isDominantFlag newMode ->
              ((newMode, newFlag), [])
          -- We need to accumulate eval flags like "-e foo -e bar"
          (Right (Right (DoEval esOld)), Right (Right (DoEval [eNew]))) ->
            ((Right (Right (DoEval (eNew : esOld))), oldFlag), errs)
          -- Saying e.g. --interactive --interactive is OK
          _ | oldFlag == newFlag -> ((oldMode, oldFlag), errs)
          -- --interactive and --show-options are used together
          (Right (Right DoInteractive), Left (ShowOptions _)) ->
            ((Left (ShowOptions True), "--interactive --show-options"), errs)
          (Left (ShowOptions _), (Right (Right DoInteractive))) ->
            ((Left (ShowOptions True), "--show-options --interactive"), errs)
          -- Otherwise, complain
          _ ->
            let err = flagMismatchErr oldFlag newFlag
             in ((oldMode, oldFlag), err : errs)
  putCmdLineState (Just modeFlag', errs', flags')
  where
    isDominantFlag f =
      isShowGhcUsageMode f
...
addFlag s flag = liftEwM $ do
  ...
  where
    loc = "addFlag by " ++ flag ++ " on the commandline"
...
doMake srcs = do
  ...
  if (null hs_srcs)
    then liftIO (oneShot hsc_env StopLn srcs)
      ...
      o_files <- mapM (\x -> liftIO $ compileFile hsc_env StopLn x) non_hs_srcs
      dflags <- GHC.getSessionDynFlags
      let dflags' =
            dflags
...
showOptions isInteractive = putStr (unlines availableOptions)
  where
    availableOptions =
      concat
        [flagsForCompletion isInteractive, map ('-' :) (getFlagNames mode_flags)]
...
showUsage ghci dflags = do
  let usage_path = if ghci then ghciUsagePath dflags else ghcUsagePath dflags
...
doFrontend modname srcs = do
  ...
  frontend
    frontend_plugin
    (reverse $ frontendPluginOpts (hsc_dflags hsc_env))
    srcs
