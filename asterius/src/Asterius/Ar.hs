...
getBSDArchEntries = do
  ...
  if is_empty then
      ...
      name    <- if CBS.unpack (CBS.take 3 name) == "#1/" then
                      liftM (CBS.unpack . CBS.takeWhile (/= '\0')) (getByteString $ read $ CBS.unpack $ CBS.drop 3 name)
                  else
                      return $ CBS.unpack $ CBS.takeWhile (/= ' ') name
