...
{-# OPTIONS_GHC -fno-cse -Wno-name-shadowing #-}

module Asterius.Unexported where
...
hscGenHardCode' :: HscEnv -> CgGuts -> ModSummary -> FilePath
               -> IO ((FilePath, Maybe FilePath, [(ForeignSrcLang, FilePath)]), Stream IO RawCmmGroup ())
               -- ^ @Just f@ <=> _stub.c is f
hscGenHardCode' hsc_env cgguts mod_summary output_filename = do
        ...
        withTiming (pure dflags)
            ...
            return ((output_filename, stub_c_exists, foreign_fps), rawcmms0)
