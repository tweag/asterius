...
$ASTERIUS_MKDIR_PATH -p $ASTERIUS_LIB_DIR/package.conf.d
$ASTERIUS_GHCPKG_PATH --package-db=$ASTERIUS_LIB_DIR/package.conf.d recache
...
cd ghc-prim
autoreconf -i
$ASTERIUS_TMP_DIR/Setup configure --prefix=$ASTERIUS_LIB_DIR --package-db=clear --package-db=global --package-db=$ASTERIUS_LIB_DIR/package.conf.d --builddir=$ASTERIUS_TMP_DIR/dist/ghc-prim --with-ghc=$ASTERIUS_AHC_PATH $ASTERIUS_CONFIGURE_OPTIONS
...
cd integer-simple
$ASTERIUS_TMP_DIR/Setup configure --prefix=$ASTERIUS_LIB_DIR --package-db=clear --package-db=global --package-db=$ASTERIUS_LIB_DIR/package.conf.d --builddir=$ASTERIUS_TMP_DIR/dist/integer-simple --with-ghc=$ASTERIUS_AHC_PATH $ASTERIUS_CONFIGURE_OPTIONS
...
cd base
autoreconf -i
$ASTERIUS_TMP_DIR/Setup configure --prefix=$ASTERIUS_LIB_DIR --package-db=clear --package-db=global --package-db=$ASTERIUS_LIB_DIR/package.conf.d --builddir=$ASTERIUS_TMP_DIR/dist/base --with-ghc=$ASTERIUS_AHC_PATH -finteger-simple $ASTERIUS_CONFIGURE_OPTIONS
