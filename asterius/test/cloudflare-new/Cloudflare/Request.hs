...
parseRequest req = do
  let _url = CBS.pack $ req_url req
      _raw_path = H.extractPath _url
      (_path_segs, _query) = H.decodePath _raw_path
      _headers = [(CI.mk $ CBS.pack k, CBS.pack v) | (k, v) <- req_headers req]
  evaluate $
    Wai.defaultRequest
      { Wai.requestMethod = CBS.pack $ req_method req,
...
req_method :: JSRequest -> String
req_method = jsonFromJSVal' . js_req_method

req_url :: JSRequest -> String
req_url = jsonFromJSVal' . js_req_url

req_headers :: JSRequest -> [(String, String)]
...
foreign import javascript unsafe "$1.method" js_req_method :: JSRequest -> JSVal

foreign import javascript unsafe "$1.url" js_req_url :: JSRequest -> JSVal

foreign import javascript unsafe "Array.from($1.headers.entries())" js_req_headers :: JSRequest -> JSVal

foreign import javascript safe "new Uint8Array($1.arrayBuffer())" js_req_body :: JSRequest -> IO JSUint8Array
